...
console.log(`Total cut tiles: ${analyses.filter(a => a !== null).length}`);
console.log(`Paired tiles: ${pairs.size}`);
console.log(`Unpaired tiles: ${analyses.filter((a, i) => a !== null && !pairs.has(i)).length}\n`);

console.log('Unpaired tiles details:');
analyses.forEach((a, i) => {
  if (a !== null && !pairs.has(i)) {
    console.log(`  Tile ${i}: area=${a.actualArea.toFixed(1)}cm² (${(a.actualArea/tileAreaCm2*100).toFixed(1)}%), bbox=${a.bb.w.toFixed(1)}×${a.bb.h.toFixed(1)}cm, ratio=${a.areaRatio.toFixed(3)}`);
  }
});

console.log('\nLooking for potential pairs among unpaired tiles:');
const unpairedIndices = [];
analyses.forEach((a, i) => {
  if (a !== null && !pairs.has(i)) {
    unpairedIndices.push(i);
...
for (let i = 0; i < unpairedIndices.length; i++) {
  ...
  for (let j = i + 1; j < unpairedIndices.length; j++) {
    ...
    const combinedArea = a1.actualArea + a2.actualArea;
    const dimDiff = Math.max(Math.abs(a1.bb.w - a2.bb.w), Math.abs(a1.bb.h - a2.bb.h));

    console.log(`  ${idx1} + ${idx2}: combined=${combinedArea.toFixed(1)}cm² (${(combinedArea/tileAreaCm2*100).toFixed(1)}%), dimDiff=${dimDiff.toFixed(2)}cm`);
  }
}
